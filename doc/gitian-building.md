...
*Setup instructions for a gitian build of Htcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Htcoin
...
- [Building Htcoin](#building-Htcoin)
...
Clone the git repositories for Htcoin and gitian and then checkout the Htcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Htcoin-project/Htcoin.git
cd Htcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Htcoin repository
...
To build Htcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Htcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Htcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Htcoin-project/Htcoin
...
For example:
```bash
URL=https://github.com/user/Htcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Htcoin=${COMMIT} --url Htcoin=${URL} ../Htcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Htcoin=${COMMIT} --url Htcoin=${URL} ../Htcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Htcoin=${COMMIT} --url Htcoin=${URL} ../Htcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Htcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Htcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Htcoin-build.assert
```
...
[Htcoin/gitian.sigs](https://github.com/Htcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
